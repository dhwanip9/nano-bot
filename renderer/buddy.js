...
async function init () {
  state.config     = await window.buddy.getConfig()
  state.blindspots = await window.buddy.getBlindspots()
  state.session    = await window.buddy.getSession()
...
function bindEvents () {
  ...
  $('link-get-key')?.addEventListener('click', e => {
    e.preventDefault()
    window.buddy.openExternal('https://console.anthropic.com/settings/keys')
  })
  ...
  $('btn-minimize')?.addEventListener('click', () => window.buddy.minimize())
  $('btn-hide')?.addEventListener('click', () => window.buddy.hide())
  ...
  $('toggle-clipboard')?.addEventListener('change', async e => {
    ...
    await window.buddy.setClipboardWatcher(enabled)
...
function bindIpcListeners () {
  ...
  window.buddy.onClipboardChanged(({ text }) => {
...
async function onboardingSubmit () {
  ...
  // Save config
  state.config = await window.buddy.saveConfig({
    apiKey,
  ...
  await window.buddy.saveSession(state.session)
...
async function saveSettings () {
  ...
  state.config = await window.buddy.saveConfig({ apiKey, projectDescription: project, skillLevel: skill })
  applyConfig()
  showScreen('main')
}
...
async function handleUserReply (cat) {
  ...
  // Build the context-rich prompt for Buddy's response
  const systemPrompt = buildSystemPrompt()
  const messages = [
    {
      role: 'user',
  ...
  const result = await window.buddy.chat({ messages, systemPrompt })
  thinking.remove()
...
function buildSystemPrompt () {
  const persona = state.blindspots.system_prompt?.buddy_persona || ''
  const rules = [
